...
export default function Charlie() {
  ...
  const toggleMic = async () => {
    ...
    if (Capacitor.isNativePlatform()) {
      if (recording) {
        // Siempre resetear el estado, aunque stop() falle porque el
        // reconocedor nativo ya se había detenido solo (p. ej. por silencio).
        try {
      ...
      try {
        ...
        SpeechRecognition.addListener("listeningState", (data) => {
          // El reconocedor nativo puede detenerse solo (silencio, timeout).
          // Cuando eso pase, refleja el estado real en la UI.
          if (data.status === "stopped") setRecording(false);
        });

        // Android tarda un instante en liberar el reconocedor anterior.
        // Si se llama a start() demasiado rápido después de un stop(),
        // falla en silencio (sin lanzar error). Un pequeño retraso
        // más un reintento evita que se quede "pegado" tras el primer uso.
        const tryStart = () => SpeechRecognition.start({ language: lang, partialResults: true, popup: false });
        await new Promise((r) => setTimeout(r, 250));
        try {
          await tryStart();
        } catch (firstErr) {
          console.warn("Reintentando iniciar micrófono:", firstErr);
