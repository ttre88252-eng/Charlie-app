...
export default function Charlie() {
  ...
  const toggleMic = async () => {
    const lang = uiLang === "es" ? "es-ES" : "en-US";

    // --- App empaquetada (Android/iOS vía Capacitor) ---
    // Patrón "de un solo disparo": igual que el navegador, donde cada
    // grabación es un evento único (habla -> transcribe -> se detiene solo).
    // Esto evita el bug de Android donde reutilizar/continuar un
    // reconocedor ya usado lo deja "ocupado" y no vuelve a arrancar.
    if (Capacitor.isNativePlatform()) {
      if (recording) {
        // El usuario tocó el botón para cancelar mientras grababa.
        try {
      ...
      try {
        ...
        if (!available) { alert(t.micUnavailable); return; }
        ...
        // Siempre limpiamos listeners viejos antes de registrar los nuevos,
        // para no acumular handlers de intentos anteriores.
        SpeechRecognition.removeAllListeners();
        ...
        // Un solo disparo: sin partialResults, sin continuous.
        // El plugin se detiene solo al terminar la frase y resuelve
        // la promesa con el resultado final — igual que el navegador.
        ...
        if (transcript) setInput(baseText + transcript);
      } catch (err) {
        console.error("Speech recognition error:", err);
        // No mostramos alerta si el usuario canceló manualmente (recording ya en false).
      } finally {
        SpeechRecognition.removeAllListeners();
        setRecording(false);
      }
    ...
    // --- Navegador web (comportamiento original) ---
