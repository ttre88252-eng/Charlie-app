      ...
      >
        ...
        {isMobile ? (
          <div style={{ borderBottom: `1px solid ${c.border}`, background: c.surfaceAlt }} className="flex-shrink-0 flex items-center justify-between px-3 py-2.5">
            ...
            <button onClick={onClose} style={{ color: c.textMuted }} className="p-1"><X size={20} /></button>
          </div>
        ) : null}
        <div
          style={
                ...
                <div style={{ borderTop: `1px solid ${c.border}` }} className="pt-3 space-y-1 max-h-56 overflow-y-auto">
                  ...
                  {conversations.map((cv) => (
                    <div key={cv.id} className="flex items-center justify-between px-2 py-1.5 rounded-lg" style={{ color: c.text }}>
                      ...
                      <button onClick={() => deleteConversation(cv.id)} style={{ color: c.danger }} title={t.deleteChat}>
                        <Trash2 size={14} />
                      </button>
                    </div>
                  ))}
            ...
            {tab === "apikey" && (
              <div>
                ...
                <div className="relative mb-3">
                  ...
                  />
                  <button onClick={() => setShowAnthropicKey((s) => !s)} style={{ color: c.textMuted }} className="absolute right-3 top-3">
                ...
                <div className="relative mb-3">
                  <input
                    ...
                    className="w-full rounded-xl p-3 pr-10 text-sm outline-none font-mono"
                  />
                  <button onClick={() => setShowGeminiKey((s) => !s)} style={{ color: c.textMuted }} className="absolute right-3 top-3">
  ...
  return (
    <div style={{ background: c.bg, color: c.text, fontFamily: "'Inter', sans-serif" }} className="w-full h-screen flex overflow-hidden">
      ...
      >
        <div style={{ width: "100%" }} className="flex flex-col h-full">
          ...
          <div className="flex-1 overflow-y-auto px-2 mt-4 space-y-0.5">
            {conversations.map((cv) => (
              <div key={cv.id} onClick={() => { setActiveId(cv.id); if (isMobile) setSidebarOpen(false); }} style={{ background: cv.id === active?.id ? c.primarySoft : "transparent", color: cv.id === active?.id ? c.primary : c.text }} className="group flex items-center gap-2 px-2.5 py-2 rounded-lg cursor-pointer text-sm mx-1">
                ...
                <button onClick={(e) => { e.stopPropagation(); deleteConversation(cv.id); }} style={{ color: c.textMuted }} className="opacity-60 md:opacity-0 md:group-hover:opacity-100 transition" title={t.deleteChat}>
                  <Trash2 size={14} />
                </button>
              </div>
            ))}
      ...
      <div className="flex-1 flex flex-col h-full min-w-0">
        <div style={{ borderBottom: `1px solid ${c.border}` }} className="flex items-center justify-between px-4 py-3 flex-shrink-0">
          <div className="flex items-center gap-3">
            <button onClick={() => setSidebarOpen((s) => !s)} style={{ color: c.textMuted }}><Menu size={19} /></button>
            <span style={{ fontFamily: "'Space Grotesk', sans-serif", color: c.text }} className="text-sm font-semibold">Charlie</span>
          </div>
          <div className="flex items-center gap-3">
            {(geminiKey || anthropicKey) && (
              <div style={{ background: c.primarySoft, color: c.primary }} className="text-xs px-2.5 py-1 rounded-full font-medium">{uiLang === "es" ? "API propia" : "Custom API"}</div>
            )}
            <button onClick={() => setSettingsOpen(true)} style={{ color: c.textMuted }} title={t.settings}><Settings size={18} /></button>
          </div>
        </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {m.role === "assistant" && m.content && (
                      <div className="flex items-center gap-3 mt-1.5 ml-0.5">
                        <button onClick={() => copyMessage(m.id, m.content)} style={{ color: c.textMuted }} title="Copiar">{copiedId === m.id ? <Check size={14} /> : <Copy size={14} />}</button>
                        <button onClick={() => regenerate(m.id)} style={{ color: c.textMuted }} title="Regenerar"><RotateCcw size={14} /></button>
                        <button style={{ color: reactions[m.id] === "up" ? c.primary : c.textMuted }} onClick={() => toggleReaction(m.id, "up")} title="Me gusta">
                          <ThumbsUp size={14} fill={reactions[m.id] === "up" ? c.primary : "none"} />
                        </button>
                        <button style={{ color: reactions[m.id] === "down" ? c.danger : c.textMuted }} onClick={() => toggleReaction(m.id, "down")} title="No me gusta">
          ...
          <div className="max-w-3xl mx-auto">
              ...
              <div className="flex flex-wrap gap-2 mb-2">
                {attachments.map((a) => (
                  <div key={a.id} style={{ background: c.surfaceAlt, border: `1px solid ${c.border}` }} className="flex items-center gap-2 pl-2.5 pr-1.5 py-1.5 rounded-lg text-xs">
                    ...
                    <button onClick={() => removeAttachment(a.id)} style={{ color: c.textMuted }}><X size={13} /></button>
                  </div>
                ))}
              </div>
            )}
            ...
            <div style={{ background: c.surface, border: `1px solid ${c.border}`, borderRadius: 20 }} className="flex items-end gap-2 px-3 py-2.5 shadow-sm relative">
              ...
              <div className="relative">
                <button onClick={() => setAttachMenuOpen((o) => !o)} style={{ color: c.textMuted }} className="p-1.5 rounded-lg hover:opacity-70 flex-shrink-0" title={t.attach}>
                  <Paperclip size={19} />
                </button>
                {attachMenuOpen && (
                  <>
              ...
              <button onClick={toggleMic} style={{ color: recording ? "#fff" : c.textMuted, background: recording ? c.danger : "transparent" }} className="p-1.5 rounded-full hover:opacity-80 flex-shrink-0 transition" title={recording ? t.stopSpeak : t.speak}><Mic size={19} /></button>

              {loading ? (
                <button onClick={stopGenerating} style={{ background: c.text, color: c.bg }} className="p-2 rounded-full flex-shrink-0" title={t.stop}><Square size={15} fill={c.bg} /></button>
              ) : (
                <button onClick={sendMessage} disabled={!input.trim() && attachments.length === 0} style={{ background: (!input.trim() && attachments.length === 0) ? c.surfaceAlt : c.primary, color: (!input.trim() && attachments.length === 0) ? c.textMuted : "#fff" }} className="p-2 rounded-full flex-shrink-0 transition" title={t.send}><Send size={15} /></button>
              )}
            </div>
